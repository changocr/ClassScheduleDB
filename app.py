...
.schedule-header-last > div {
    border-right: 1px solid #ccc !important;
}
.schedule-header-row .schedule-header {
...
.schedule-time-last > div {
    border-bottom: 1px solid #ccc !important;
}
/* 颜色选择区紧凑样式 */
.color-select-btn > button {
...
if user_name.lower() == "admin":
    ...
    # 渲染空闲表
    header_cols2 = st.columns([1] + [1]*len(WEEKDAYS), gap=0)
    ...
    # 紧凑横向排列，无间距
    select_cols = st.columns(len(color_options), gap="small")
    ...
    with st.container():
        ...
        # 表头行
        header_cols = st.columns([1.2] + [1]*len(WEEKDAYS), gap=0)
        header_cols[0].markdown(f'<div class="schedule-header schedule-header-row">时间</div>', unsafe_allow_html=True)
        for d, day in enumerate(WEEKDAYS):
            last_class = "schedule-header-last" if d == len(WEEKDAYS)-1 else ""
            header_cols[d+1].markdown(f'<div class="schedule-header schedule-header-row {last_class}">{day}</div>', unsafe_allow_html=True)
        ...
        for row_idx, period in enumerate(PERIODS):
            ...
            row_cols = st.columns([1.2] + [1]*len(WEEKDAYS), gap=0)
