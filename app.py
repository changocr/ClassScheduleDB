...
<style>
/* 课表容器：微调间距，实现视觉无缝 */
.schedule-wrapper [data-testid="stHorizontalBlock"] {
    margin-bottom: -1px !important; /* 关键：让行与行重叠1px，消除边框间隙 */
}
...
/* 表头和时间列 */
.schedule-header-cell, .schedule-time-cell {
    height: 36px !important;
    line-height: 36px !important;
    margin: 0 !important;
...
.schedule-header-cell p, .schedule-time-cell p {
    margin: 0 !important;
    padding: 0 !important;
}
</style>
...
if user_name.lower() == "admin":
    ...
    for period in PERIODS:
        ...
        for d, day in enumerate(WEEKDAYS):
            ...
            row_cols[d+1].markdown(
                f"<div style='background-color:{bg_color}; padding:10px; border-radius:0; border:1px solid #ccc; border-right:none; text-align:center; font-weight:bold; height:36px; line-height:16px;'>{label}</div>",
                unsafe_allow_html=True
            )
    ...
    for idx, week_info in enumerate(week_options):
        with select_cols[idx]:
            ...
            if st.button(
                week_info["label"],
                key=f"sel_{week_info['record_id']}",
    ...
    # 课表容器
    with st.container():
        st.markdown('<div class="schedule-wrapper">', unsafe_allow_html=True)
        
        # 表头行
        header_cols = st.columns([1.2] + [1]*len(WEEKDAYS))
        header_cols[0].markdown(f'<div class="schedule-time-cell">时间</div>', unsafe_allow_html=True)
        for d, day in enumerate(WEEKDAYS):
            header_cols[d+1].markdown(f'<div class="schedule-header-cell">{day}</div>', unsafe_allow_html=True)
        ...
        for row_idx, period in enumerate(PERIODS):
            ...
            # 时间列
            row_cols[0].markdown(f'<div class="schedule-time-cell">{period}</div>', unsafe_allow_html=True)
            ...
            for d, day in enumerate(WEEKDAYS):
                ...
                # 渲染按钮
                with row_cols[d+1]:
                    st.markdown('<div class="schedule-cell">', unsafe_allow_html=True)
