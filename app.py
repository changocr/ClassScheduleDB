...
/* 核心：干掉Streamlit默认的垂直容器间距（行间距最大元凶） */
#schedule-root [data-testid="stVerticalBlock"] {
    gap: 0 !important;
}

/* 干掉水平列容器的间距 */
#schedule-root [data-testid="stHorizontalBlock"] {
    gap: 0 !important;
}

/* 干掉列内所有嵌套容器的间距 */
#schedule-root [data-testid="column"] > div {
    width: 100% !important;
}
...
/* 干掉markdown默认边距 */
.schedule-header p, .schedule-time p {
    margin: 0 !important;
    line-height: 34px !important;
}
</style>
...
if user_name.lower() == "admin":
    ...
    # 课表根容器（所有零间距样式只作用在这里）
    st.markdown('<div id="schedule-root">', unsafe_allow_html=True)
