...
# ============== 核心：终极零间距CSS（强制覆盖所有Streamlit默认样式） ==============
st.markdown("""
<style>
/* 课表容器唯一ID，样式完全隔离，不影响其他页面 */
...
/* 强制消除所有行容器的间距 */
#student-schedule-table [data-testid="stHorizontalBlock"] {
    margin: 0 !important;
    padding: 0 !important;
    gap: 0 !important;
...
/* 强制消除列内所有垂直间距（核心：干掉行间距的元凶） */
#student-schedule-table [data-testid="column"] {
    margin: 0 !important;
    padding: 0 !important;
    min-height: unset !important;
}
#student-schedule-table [data-testid="column"] > div {
    margin: 0 !important;
    padding: 0 !important;
    min-height: unset !important;
}
#student-schedule-table [data-testid="stVerticalBlock"] {
    margin: 0 !important;
    padding: 0 !important;
    gap: 0 !important;
    min-height: unset !important;
...
/* 干掉markdown默认边距 */
.schedule-time p, .schedule-header p {
    margin: 0 !important;
    padding: 0 !important;
    line-height: 36px !important;
}
...
.schedule-row-last .schedule-time {
    border-bottom: 1px solid #ccc !important;
}
...
if user_name.lower() == "admin":
    ...
    # 课表专属容器，所有样式限定在这里
    st.markdown('<div id="student-schedule-table">', unsafe_allow_html=True)
